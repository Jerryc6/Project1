...
if (isset($_GET['bi_lls'])) {
    # code...
    $bi_lls=$_GET['bi_lls'];
    $delete=mysqli_query($connn,"DELETE FROM bill_rates WHERE bi_lls = '$bi_lls'");
...
<form action="connection_tenants.php" method="post" name="myForm" autocomplete="off">
<div class="questions">
    <input type="text" placeholder="Bills"  id="bi_lls" name="bi_lls" required value="">
    <input type="text" placeholder="Rates" id="ra_te" name="ra_te" required value="">
    
    <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        <button type="submit" class="btn btn-primary" onclick="Myfunction()" name="save_billrates">Save changes</button>
    </div>
</div>
...
<div class="modal fade" id="updateDataq" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">ADD BILLRATES</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<div class="side-bar">
   ...
   <div class="profile">
    <h3 class="name"><i class="fa-solid fa-building"></i>  BHMS</h3>
   </div>
   <nav class="navbar">
...
<section class="courses">
 <h1 class="heading"><i class="fa-solid fa-plug"></i>  Bill Rates</h1>
            ...
            <tbody>
                  <?Php
                   ...
                   if (isset($_GET['search'])) {
                    ...
                    $query = "SELECT * FROM bill_rates WHERE CONCAT (bi_lls,ra_te) LIKE '%$filtervalues%' ";
                    $query_run = mysqli_query($connn, $query);
                     if (mysqli_num_rows($query_run)>0) {
                      ...
                      <td>
                        <a href="billrates_page.php?bi_lls=<?=$bed['bi_lls'];?>" class="" ><i class="fa fa-trash"></i></a>
                      </td>
                        </tr>
                      <?php 
                      }
                    ...
                    foreach ($query_runn as $row) {
                      ...
                      <td><?php echo $row['bi_lls'];?></td>
                      <td><?php echo $row['ra_te'];?></td>
                      <td>
                        <button type="button" class="editbtn" data-toggle="modal" data-target="#updateData">
                        <i class="fa fa-pencil"></i>
                        </button>
                      ...
                      <td>
                        <a href="billrates_page.php?bi_lls=<?=$row['bi_lls'];?>" class="" ><i class="fa fa-trash"></i></a>
                      </td>
                      </tr>
                      <?php
                    }
